...
def save_weights(epoch, net, optimizer, save_path, model_name):
    ...
    if epoch % 1 == 0:
        torch.save(
            checkpoint, os.path.join(save_path, "%d_%s.pth" % (epoch, model_name))
        )
...
def save_prediction2(
    p0, p1, lb0, lb1, scores0, scores1, path, pc0_name, pc1_name, data_path
):
    ...
    if tcfg.remove_plane:
        plane0 = np.loadtxt(
            osp.join(
                data_path, path.split("\\")[-1], p0_name.replace(".txt", "_plane.txt")
            ),
            usecols=(0, 1, 2, -1),
        )
    ...
    if tcfg.remove_plane:
        plane1 = np.loadtxt(
            osp.join(
                data_path, path.split("\\")[-1], p1_name.replace(".txt", "_plane.txt")
            ),
            usecols=(0, 1, 2, -1),
        )
