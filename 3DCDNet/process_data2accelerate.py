    ...
    def load_and_pp_data(self):
        for idx in tqdm(range(self.file_size)):
            query_path = os.path.join(
                tcfg.path.prepare_data, self.flag, str(idx) + ".npy"
            )
            ...
            p16_path = osp.join(self.data_path, dir_name, pc0_name)
            p20_path = osp.join(self.data_path, dir_name, pc1_name)
            ...
            ppdata = np.asarray(ppdata, dtype="object")
...
def prepare_data_train():
    ...
    pptrain = PrepareData(
        "train", tcfg.path["train_dataset"], tcfg.path["train_txt"], tcfg.n_samples
    )
    ppval = PrepareData(
        "val", tcfg.path["val_dataset"], tcfg.path["val_txt"], tcfg.n_samples
    )
...
def prepare_data_test():
    ...
    pptest = PrepareData(
        "test", tcfg.path["test_dataset"], tcfg.path["test_txt"], tcfg.n_samples
    )
