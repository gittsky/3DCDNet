...
from process_data2accelerate import prepare_data_train, prepare_data_test, norm_data
from RandomSampler import (
    generate_removed_plane_dataset_train,
    generate_removed_plane_dataset_test,
)
...
class CDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if not tcfg.if_prepare_data:
            ...
            ppdata = np.load(
                os.path.join(self.ppdata_path, self.flag, str(idx) + ".npy"),
                allow_pickle=True,
            )
            inputs16, inputs20, dir_name, pc0_name, pc1_name, [p16_raw, p20_raw] = (
                ppdata
            )
            if not tcfg.use_rgb:
                inputs16["xyz"] = [i[:, :3] for i in inputs16["xyz"]]
